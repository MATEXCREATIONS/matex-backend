...
function buildChatNotificationHtml(conversation, message, options = {}) {
  const title = options.forAdmin ? 'New Support Message' : 'Reply from Matex Team';
  return `
    <div style="font-family: Arial, sans-serif; max-width: 680px; margin: 0 auto; background: #f5f5f5; padding: 24px; border-radius: 12px;">
      <h2 style="color: #8b0000; text-align: center;">${title}</h2>
      <div style="background: white; padding: 20px; border-radius: 10px; margin-top: 18px;">
        <p><strong>Conversation:</strong> ${conversation.subject || 'Support request'}</p>
      ...
      <p style="color: #666; font-size: 13px; margin-top: 20px;">This message was generated by the Matex support system.</p>
    </div>
  `;
}
...
async function notifyCustomerAboutAdminReply(conversation, message) {
  ...
  try {
    await sendEmail(
      ...
      `Reply from Matex — ${conversation.subject || 'Support request'}`,
 ...
 * Return admin history entries for an order
 */
app.get('/api/admin/orders/:orderId/history', adminAuth, async (req, res) => {
  try {
    const orderId = String(req.params.orderId || '').trim();
    if (!orderId) return res.status(400).json({ success: false, message: 'orderId is required' });
    if (supabase) {
      const { data, error } = await supabase.from('matex_history').select('*').eq('order_id', orderId).order('created_at', { ascending: false });
      if (error) {
        console.error('Supabase fetch history error:', error);
        return res.status(500).json({ success: false, message: 'Unable to load history' });
      }
      return res.json({ success: true, history: data || [] });
    }

    // fallback to in-memory history store
    const histories = (historyStore && historyStore.get(orderId)) || [];
    return res.json({ success: true, history: Array.isArray(histories) ? histories : [] });
  } catch (err) {
    console.error('Admin history fetch error:', err.message || err);
    return res.status(500).json({ success: false, message: 'Unable to load history' });
  }
});

/**
 * POST /api/admin/orders/:orderId/history
 * Add an admin history entry for an order
 */
app.post('/api/admin/orders/:orderId/history', adminAuth, async (req, res) => {
  try {
    const orderId = String(req.params.orderId || '').trim();
    if (!orderId) return res.status(400).json({ success: false, message: 'orderId is required' });
    const { action = '', notes = '', admin_name = null, admin_id = null, metadata = null } = req.body || {};
    const record = {
      order_id: orderId,
    ...
    if (supabase) {
      try {
        const { data, error } = await supabase.from('matex_history').insert([record]).select();
        if (error) {
          console.error('Supabase insert history error:', error);
        }
        // Also append to matex_orders.status_history for quick timeline visibility
        try {
          const existingRes = await supabase.from('matex_orders').select('status_history').eq('order_id', orderId).limit(1).maybeSingle();
          const existing = existingRes && !existingRes.error ? existingRes.data || null : null;
          const existingHistory = Array.isArray(existing?.status_history) ? existing.status_history : [];
          const historyEntry = { status: record.action || 'Admin action', message: record.notes || '', updated_at: record.created_at };
          await supabase.from('matex_orders').update({ status_history: [...existingHistory, historyEntry], latest_progress: historyEntry.message || historyEntry.status }).eq('order_id', orderId);
        } catch (e) {
          console.warn('Failed to append admin history to order status_history', e && e.message);
        }
        return res.json({ success: true, entry: Array.isArray(data) && data.length ? data[0] : record });
      } catch (err) {
        console.error('Supabase history insert exception:', err && err.message);
      }
    }

    // fallback: in-memory history store
    const existing = historyStore.get(orderId) || [];
    const list = [record, ...existing];
    historyStore.set(orderId, list);
    return res.json({ success: true, entry: record });
  } catch (err) {
    console.error('Admin history insert error:', err.message || err);
    return res.status(500).json({ success: false, message: 'Unable to save history entry' });
  }
});

/**
 * PUT /api/admin/orders/:orderId/download
 * Toggle download access for an order
 */
app.put('/api/admin/orders/:orderId/download', adminAuth, async (req, res) => {
  try {
    const orderId = String(req.params.orderId || '').trim();
    if (!orderId) return res.status(400).json({ success: false, message: 'orderId is required' });
    const downloadAccess = Boolean(req.body && typeof req.body.download_access !== 'undefined' ? req.body.download_access : req.body);
    let updatedOrder = null;
    if (supabase) {
      try {
        const { data, error } = await supabase.from('matex_orders').update({ download_access: downloadAccess }).eq('order_id', orderId).select().limit(1).single();
        if (error) {
          console.error('Supabase update download_access error:', error);
        } else {
          updatedOrder = data;
        }
      } catch (err) {
        console.warn('Supabase update download_access exception:', err && err.message);
      }
    }

    if (!updatedOrder) {
      const existing = orderStore.get(orderId) || null;
      if (!existing) return res.status(404).json({ success: false, message: 'Order not found' });
      existing.download_access = downloadAccess;
      orderStore.set(orderId, existing);
      updatedOrder = existing;
    }

    // Persist history entry about unlocking downloads
    try {
      const hist = { action: downloadAccess ? 'Downloads Unlocked' : 'Downloads Locked', notes: `Admin set download_access=${downloadAccess}`, created_at: new Date().toISOString() };
      if (supabase) {
        await supabase.from('matex_history').insert([{ order_id: orderId, action: hist.action, notes: hist.notes, created_at: hist.created_at }]);
        const existingRes = await supabase.from('matex_orders').select('status_history').eq('order_id', orderId).limit(1).maybeSingle();
        const existing = existingRes && !existingRes.error ? existingRes.data || null : null;
        const existingHistory = Array.isArray(existing?.status_history) ? existing.status_history : [];
        await supabase.from('matex_orders').update({ status_history: [...existingHistory, hist], latest_progress: hist.notes }).eq('order_id', orderId);
      } else {
        const existing = orderStore.get(orderId) || {};
        const histList = Array.isArray(existing.status_history) ? [...existing.status_history, hist] : [hist];
        existing.status_history = histList;
        orderStore.set(orderId, existing);
      }
    } catch (e) {
      console.warn('Failed to persist download toggle history', e && e.message);
    }

    try { broadcastAdminEvent('order', updatedOrder); } catch (e) {}
    return res.json({ success: true, order: updatedOrder });
  } catch (err) {
    console.error('Download toggle error:', err && err.message);
    return res.status(500).json({ success: false, message: 'Unable to set download access' });
  }
});
...
app.post('/api/chat/conversations', async (req, res) => {
  ...
  try {
    ...
    const conversationData = {
      ...
      subject: String(subject || 'Support inquiry'),
      status: 'open',
      source: 'website',
